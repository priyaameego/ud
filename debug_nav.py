...
filepath = r"c:\Users\Priya\Documents\udyog\ayush-export-promotion-council.html"
...
# Also remove the single row logo/login if they exist
if '<!-- Navbar 2 Logo -->' in content:
    print("Found Navbar 2 Logo!")
    content = re.sub(r'<div class="hidden lg:flex items-center gap-2 w-full justify-between pr-4 desktop-nav text-sm font-semibold text-gray-700">\s*<!-- Navbar 2 Logo -->\s*<a href="index\.html" class="flex flex-col mr-2 shrink-0">.*?</a>', '<div class="hidden lg:flex items-center gap-4 w-full justify-between pr-4 desktop-nav text-sm font-semibold text-gray-700">', content, flags=re.DOTALL)
else:
    print("Navbar 2 Logo not found!")

if '<!-- Support & Login -->' in content:
    print("Found Support & Login!")
    content = re.sub(r'</a>\s*<!-- Support & Login -->\s*<div class="flex items-center gap-3 ml-2 shrink-0">.*?</div>\s*</div>', '</a>\n        </div>', content, flags=re.DOTALL)
else:
    print("Support & Login not found!")

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)

print("Done writing to ayush-export-promotion-council.html")
